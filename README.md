...
次引入其他文件   以上使用一行分隔
## 使用css module
css 使用 less 命名为 ***.module.less
引入 import styles from './***.module.less'   使用 styles.***
className命名为驼峰命名

## tslint配置
  "rules":{
    "no-console":false,
    "ordered-imports": false, import引入要按字母顺序
    "jsx-self-close": false,  没有子元素的JSX元素必须是自关闭的
  }

## reset.less  全局样式
src/common/reset.less
存放全局样式以及常用的样式，用来减少在工作中写样式的代码量
例如className=".mgl10" 代表margin-left:10px     更多请自行查看
